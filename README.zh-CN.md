...
将可信信息流转化为日报与可研究 evidence 的 AI Agent 工具包。

DigestPilot 不是独立 RSS 应用，而是给 Agent 使用的信息流聚合、日报生成与研究工作流套件。Agent 应先加载对应的 `SKILL.md`，按需读取 references，用确定性 CLI 准备内容、evidence 和状态，最后由 Agent 写出用户需要的 digest、趋势卡片、源评估或研究综合。
...
DigestPilot 已包含生成式 Claude Code 与 Codex 插件包：[`plugins/digestpilot`](./plugins/digestpilot/README.md)。根目录 [`skills/`](./skills) 仍然是 Skill 的唯一来源。
...
- 在核心 CLI 或 Skill 契约中绑定特定 runtime。
...
- [插件打包路线图](./docs/plugin-packaging-roadmap.md)
